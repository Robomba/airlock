...
policy, Stopgate watches their OWN past sessions and writes an allowlist of what
...
Two hard rules keep this honest and safe (v1 is still observe-only):
  ...
  2. **Stopgate will not learn an activity it flagged.** When learning, any
     ...
     teaches Stopgate that the exfil domain is "normal". The learned policy is
...
class LearnStats:
    ...
    excluded: int = 0      # actions Stopgate refused to learn (flagged/risky)


def _harvestable(decision: Decision) -> bool:
    """Whether an action may teach the policy. We refuse to learn from anything
    Stopgate flagged as suspicious or that is irreversible — so an exfil session
...
def default_policy_path() -> str:
    ...
    ``./.stopgate.toml`` when it already exists, else the per-user file."""
    local = os.path.join(os.getcwd(), ".stopgate.toml")
    if os.path.exists(local):
        return local
    return os.path.join(os.path.expanduser("~"), ".stopgate", "policy.toml")


def discover_policy_path() -> Optional[str]:
    """Return an existing policy file to auto-load, or ``None`` (zero-config).

    Explicit env override first (``STOPGATE_POLICY``), then project-local, then
    the per-user file. Never invents a path that doesn't exist, so the default
    behaviour with no learned policy is unchanged.
    """
    env = os.environ.get("STOPGATE_POLICY")
    if env and os.path.exists(env):
        return env
    for cand in (
        os.path.join(os.getcwd(), ".stopgate.toml"),
        os.path.join(os.path.expanduser("~"), ".stopgate", "policy.toml"),
    ):
...
def render_policy(policy: Policy) -> str:
    ...
    L: List[str] = []
    L.append("# Stopgate auto-learned policy  —  generated by `stopgate learn`")
    L.append("#")
    L.append("# Observe-only. Stopgate uses this to SUPPRESS known-normal activity so the")
    L.append("# signal in `stopgate report` / `stopgate digest` is the anomalies, not your")
...
def _parse_toml(raw: bytes) -> Dict[str, object]:
    ...
    the exact subset ``stopgate learn`` emits (sections, string values, and
